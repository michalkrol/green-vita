...
/// Experiment gate: metadata reports OFF (keep only the 5 s gamepad keepalive).
/// At ~12 pkt/s of metadata the drift matched the 62 Hz gamepad build (~1 s per
/// 4 min), while the ONLY zero-drift config ever observed sent no input-channel
/// packets at all — so metadata CONTENT, not packet rate, is the remaining
/// poison suspect. Re-enable once report timing semantics match the reference.
    ...
    queue
        ...
        .map(|queued| {
            ...
            MetadataFrame {
                ...
                // Floor-era semantics: Chrome-parity deltas (+3/+45/+330 ms,
                // mimicking its webrtc dump) made drift WORSE (~1 s/min vs the
                // 0.25-0.5 s/min floor), so the console does not simply size
                // buffers off these reports — minimal values test best.
