...
///
/// The console honors `b=AS`/`b=TIAS` on the offered video m-line as its encoder
/// target ceiling. Without any cap (the rtc crate emits none) the encoder is free
/// to run past what it can sustain at 60 fps and its pre-stamp queue grows
/// unboundedly, which presents as linear glass-to-glass latency drift. Reference
/// clients always signal a cap (Greenlight locks 15 Mbps; verified console
/// behavior in DRHATL95/kite#59).
/// Greenlight-parity cap. Rationale: the Vita's 802.11n link realistically
/// absorbs ~20-30 Mbps; at a 50M cap scene-change bursts overshoot the link,
/// the console's socket buffer grows, and its pacer never drains the backlog
/// (linear glass-lag drift during active content only). A cap below the link
/// ceiling keeps bursts inside headroom; REMB matches this value.
pub(super) const VIDEO_BITRATE_CAP_KBPS: u32 = 15_000;

/// Injects `b=AS`/`b=TIAS` bandwidth lines into the H.264 video media section,
/// replacing any existing ones there. No-op when no such section exists.
